...
import static com.piotrpoleszak.loginAndRegistration.exception.ErrorSubcode.*;
...
public class RegistrationService {
    ...
    public String register(RegistrationRequest request) {
        ...
        var link = "http://localhost:8080/api/v1/registration/confirm?token=" + token;
    ...
    public String confirmToken(String token) {
        var confirmationToken = confirmationTokenService
                .getToken(token)
                .orElseThrow(() -> new CoreException(VALIDATE_ERROR, TOKEN_NOT_FOUND));
        var expiredAt = confirmationToken.getExpiresAt();

        if (confirmationToken.getConfirmedAt() != null) {
            throw new CoreException(VALIDATE_ERROR, EMAIL_ALREADY_CONFIRMED);
        }
        if (expiredAt.isBefore(LocalDateTime.now())) {
            throw new CoreException(VALIDATE_ERROR, TOKEN_EXPIRED);
        }
